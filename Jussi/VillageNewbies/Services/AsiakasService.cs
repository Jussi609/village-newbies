...
namespace VillageNewbies.Services
{
    public class AsiakasService
    {
        ...
        {
            string query = "SELECT * FROM asiakas WHERE etunimi LIKE @searchTerm OR sukunimi LIKE @searchTerm OR email LIKE @searchTerm OR puhelinnro LIKE @searchTerm ORDER BY sukunimi, etunimi";
            var parameters = new Dictionary<string, object>
            {
        ...
        {
            // Tarkista ensin, onko postinumero olemassa
            if (!await PostinumeroExistsAsync(asiakas.Postinumero))
            {
                // Postinumeroa ei löydy - ei voida lisätä asiakasta
                return -2; // Virhekoodi: postinumero ei ole olemassa
            }
        ...
        private async Task<bool> PostinumeroExistsAsync(string postinumero)
        {
